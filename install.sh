...
# Hysteria v1 Easy Installer
# ===========================
# Run: bash <(curl -sL http://82.26.104.48:8888/hysteria-install.sh)

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'; NC='\033[0m'

echo -e "${CYAN}╔═══════════════════════════════════╗${NC}"
echo -e "${CYAN}║     Hysteria v1 Easy Installer    ║${NC}"
echo -e "${CYAN}╚═══════════════════════════════════╝${NC}"
echo ""

# Root check
if [ "$EUID" -ne 0 ]; then echo -e "${RED}Please run as root (sudo su)${NC}"; exit 1; fi

# Config
IP=$(curl -s ifconfig.me)
read -p "Port (default: 25000): " PORT
PORT=${PORT:-25000}
read -p "AUTH Password (default: naman): " AUTH
AUTH=${AUTH:-naman}
...
cat > /etc/systemd/system/hysteria.service << 'SYSTEMD'
[Unit]
