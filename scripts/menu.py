...
def count_ssh():
    try:
        # นับเฉพาะคนล็อกอินสำเร็จแล้วจริงๆ (มี @ และมี pts/tty) ไม่นับ bot/priv/notty/net
        r = subprocess.run("ps -eo args | grep -E '[s]shd: [^ ]+@' | grep -E 'pts|tty' | wc -l", shell=True, capture_output=True,text=True,timeout=3)
