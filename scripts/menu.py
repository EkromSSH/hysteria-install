...
def count_ssh():
    try:
        ...
        # พอร์ตที่เครื่องฟังอยู่ ยกเว้น 22 และพอร์ตเว็บ(80/443/53/82)
        lports = subprocess.run("ss -tlnp 2>/dev/null | awk '{print $4}' | grep -oE ':[0-9]+$' | tr -d ':' | grep -vE '^(22|80|443|53|82)$' | sort -u", shell=True, capture_output=True,text=True,timeout=3).stdout.strip().split()
        ips = set()
...
def count_udp():
    ...
    try:
        ...
        ips = set()
        my_ip = subprocess.run("ip -o -4 route get 8.8.8.8 | awk '{print }'", shell=True, capture_output=True,text=True,timeout=3).stdout.strip()
...
def show_info():
    ...
    for l,v in [("Protocol","UDP Hysteria v1"),("Server",ip),("Port",f"{p} (10000-65000)"),
