...
# เอารายการพอร์ตที่เครื่องเราฟังอยู่ (listening) ยกเว้น 22 และพอร์ตเว็บ(80/443/53/82 ฯลฯ ของตัวเอง)
LPORTS=$(ss -tlnp 2>/dev/null | awk '{print $4}' | grep -oE ':[0-9]+$' | tr -d ':' | grep -vE '^(22|80|443|53|82)$' | sort -u | tr '\n' '|')
LPORTS="${LPORTS%|}"
SSH_ON=0
